...
/**
 * 🟡 SERVICE LAYER - Business Logic for System Reference Data
 */
export const systemService = {
	/**
	 * Calculate pediatric dosage based on weight
	 */
	async calculatePediatricDosage(
		drugName: string,
		weightKg: number,
		...
		if (clinicalIndication) {
			guidelines = guidelines.filter(
				(g: { clinicalIndication: string }) =>
					g.clinicalIndication.toLowerCase() ===
					clinicalIndication.toLowerCase()
			)
		}
	...
	async calculateZScore(
		...
		if (lower.ageDays === ageDays) {
			return this.calculateZScoreFromStandard(
	...
	/**
	 * Get growth classification based on Z-score
	 */
	classifyGrowth(chartType: ChartType, zScore: number): string {
		switch (chartType) {
			...
			.filter(
				(vaccine: {
					...
					if (vaccine.ageInDaysMin && ageDays < vaccine.ageInDaysMin)
						return false
					if (vaccine.ageInDaysMax && ageDays > vaccine.ageInDaysMax)
						return false
