...
import { calculateZScore, getAgeInDays, type LMSDataPoint } from '@/utils'
...
class GrowthService {
	...
	async getGrowthRecordsByPatient(
		patientId: string,
		clinicId: string,
	...
	async calculateZScores(
		...
		'use cache'
		cacheTag(`zscore:calc:${gender}:${ageDays}`)
		cacheLife(CACHE_PROFILES.medicalShort)

		const growthDataMap = await this.getWHOStandardsMap()
		const result = calculateZScore(growthDataMap, gender, ageDays, weight)
	...
	async calculateMultipleZScores(
		...
		const results = await Promise.all(
			measurements.map(m =>
				this.calculateZScores(m.ageDays, m.weight, m.gender)
			)
		)
	...
	async getPatientZScoreChart(
		...
		const [chartData, measurements] = await Promise.all([
			this.getZScoreChartData(patient.gender as 'MALE' | 'FEMALE', chartType),
	...
	async getZScoreAreas(
		...
		const chartData = await this.getZScoreChartData(gender, chartType)
	...
	async createGrowthRecord(input: CreateGrowthRecordInput, userId: string) {
		// Verify clinic access
		await validateClinicAccess(input.clinicId ?? '', userId)

		// Verify patient exists and belongs to clinic
		const patient = await growthQueries.checkPatientExists(input.patientId)
		if (!patient) {
			throw new TRPCError({
				code: 'NOT_FOUND',
				message: 'Patient not found',
		...
		const weightZScore = input.weight
			? calculateZScore(growthDataMap, gender, ageDays, input.weight)
			: null

		const heightZScore = input.height
			? calculateZScore(growthDataMap, gender, ageDays, input.height)
			: null
		...
		// Cache invalidation
		cacheHelpers.patient.invalidateMedical(input.patientId, input.clinicId)
