...
export async function getVisitById(id: string, clinicId: string) {
...
export async function getVisitsByClinic(
	clinicId: string,
	options?: {
		limit?: number
		offset?: number
...
export async function getRecentVisits(clinicId: string, limit = 5) {
	return visitQueries.findRecent(clinicId, limit)
}

export async function getTodayVisits(clinicId: string) {
...
export async function getTodayVisitCount(clinicId: string) {
	return visitQueries.countToday(clinicId)
}

export async function getMonthVisitCount(clinicId: string) {
...
export async function createVisit(input: VisitCreateInput, userId: string) {
	...
	if (input.time) {
		await validateWorkingHours(doctor, input.time)
	}
	...
	cacheHelpers.visit.invalidateDoctorSchedule(input.doctorId, patient.clinicId)
...
export async function cancelVisit(id: string, userId: string) {
	return updateVisitStatus(id, 'CANCELLED', userId)
}

export async function completeVisit(id: string, userId: string) {
	return updateVisitStatus(id, 'COMPLETED', userId)
}
