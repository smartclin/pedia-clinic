...
class VaccinationService {
	...
	async getImmunizationsByClinic(
		clinicId: string,
		options?: {
	...
	async getUpcomingVaccinationCount(clinicId: string, daysAhead = 30) {
		return vaccinationQueries.countUpcoming(clinicId, daysAhead)
	}

	async getOverdueVaccinationCount(clinicId: string, daysOverdue = 0) {
		return vaccinationQueries.countOverdue(clinicId, daysOverdue)
	}

	async getVaccinationCountByStatus(
		clinicId: string,
		status: ImmunizationStatus
	) {
	...
	async getVaccineSchedule(options?: VaccineScheduleFilterInput) {
		return vaccinationQueries.findVaccineSchedule(options)
	}

	async getVaccineScheduleByAge(ageMonths: number) {
		return vaccinationQueries.findVaccineScheduleByAge(ageMonths)
	}

	async calculateDueVaccinations(patientId: string, clinicId: string) {
		// 1. Verify patient
		const patient = await this.verifyPatientAccess(patientId, clinicId)
		...
		const schedule =
			await vaccinationQueries.findVaccineScheduleByAge(ageMonths)
	...
	async getPatientRecord(patientId: string, clinicId: string) {
		// 1. Permission check
		await this.verifyPatientAccess(patientId, clinicId)
	...
	async getDueVaccinations(patientId: string, clinicId: string) {
		const patient = await this.verifyPatientAccess(patientId, clinicId)
		...
		return schedule
			...
			.map(
				(s: { ageInDaysMin: number | null; ageInDaysMax: number | null }) => {
	...
	async getClinicStats(clinicId: string, startDate?: Date, endDate?: Date) {
	...
	async deleteImmunization(id: string, clinicId: string, userId: string) {
		// 1. Check access
	...
	async recordImmunization(input: ImmunizationCreateInput, userId: string) {
		// 1. Validate patient
	...
	async scheduleVaccination(input: ScheduleVaccinationInput, userId: string) {
		// 1. Validate patient
		const patient = await vaccinationQueries.checkPatientExists(
			input.patientId,
			input.clinicId ?? ''
	...
	async getPatientVaccinationSummary(patientId: string, clinicId: string) {
		await this.verifyPatientAccess(patientId, clinicId)
