...
export async function getServiceById(id: string, clinicId: string) {
...
export async function getServicesByClinic(
	clinicId: string,
...
export async function updateService(input: UpdateServiceInput, userId: string) {
...
export async function softDeleteService(
	...
	const service = await serviceQueries.softDeleteService(id, new Date(), reason)
...
export async function restoreService(id: string, userId: string) {
	// Get existing service
	const existing = await serviceQueries.findServiceById(id)
	if (!existing) {
		throw new TRPCError({
...
export async function permanentlyDeleteService(id: string, userId: string) {
	// Get existing service
	const existing = await serviceQueries.findServiceById(id)
	if (!existing) {
		throw new TRPCError({
