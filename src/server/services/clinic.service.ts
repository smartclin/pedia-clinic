...
export class ClinicService {
	...
	async getClinicWithUserAccess(clinicId: string, userId: string) {
		...
		const access = await clinicQueries.findClinicWithUserAccess(
			clinicId,
			userId
		)
		if (!access) {
	...
	async getPersonalizedGreeting(userPrefs?: {
		lastVisit?: string
		childName?: string
	}) {
		const now = new Date()
...
export class DashboardService {
	async getDashboardStats(clinicId: string, from: Date, to: Date) {
		'use cache'
		cacheTag(
	...
	async getMedicalRecordsSummary(clinicId: string) {
		'use cache'
	...
	async getRecentAppointments(clinicId: string) {
		'use cache'
	...
	async getMonthlyPerformance(clinicId: string) {
		'use cache'
