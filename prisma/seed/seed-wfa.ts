...
import { ChartType, Gender, MeasurementType } from '@/types';
...
export default async function wfaSeed(prisma: PrismaSeedClient) {
  ...
  if (whoJson.wfa.boys && Array.isArray(whoJson.wfa.boys)) {
    ...
    for (const dataPoint of whoJson.wfa.boys) {
      try {
        const mappedRecord = mapDataPoint(dataPoint, MeasurementType.Weight, ChartType.WFA, Gender.MALE);
  ...
  if (whoJson.wfa.girls && Array.isArray(whoJson.wfa.girls)) {
    ...
    for (const dataPoint of whoJson.wfa.girls) {
      try {
        const mappedRecord = mapPoint(dataPoint, MeasurementType.Weight, ChartType.WFA, Gender.FEMALE);
        allRecords.push(mappedRecord);
  ...
  for (const record of allRecords) {
    ...
    const key = `${record.gender}:${record.measurementType}:${record.chartType}:${record.ageDays}`;
...
function mapPoint(
  ...
  return mapDataPoint(dataPoint, measurementType, chartType, gender);
}
