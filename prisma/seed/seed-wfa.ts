...
export default async function wfaSeed(prisma: PrismaSeedClient) {
	...
	try {
		...
		console.error(
			`❌ Error reading or parsing WHO data file at ${ZSCORE_DATA_PATH}.`
		)
	...
	if (whoJson.wfa.boys && Array.isArray(whoJson.wfa.boys)) {
		...
		console.log(
			`    ✅ Processed: ${boysProcessed}, ❌ Skipped: ${boysSkipped}`
		)
	}
	...
	if (whoJson.wfa.girls && Array.isArray(whoJson.wfa.girls)) {
		...
		console.log(
			`    ✅ Processed: ${girlsProcessed}, ❌ Skipped: ${girlsSkipped}`
		)
	}
	...
	for (const record of allRecords) {
		...
		if (uniqueRecordsMap.has(key)) {
			duplicates.push(key)
			console.warn(
				`⚠️ Duplicate found: ${key} (Age: ${record.ageInMonths} months)`
			)
		} else {
	...
	if (dataToInsert.length > 0) {
		try {
			...
			for (let i = 0; i < dataToInsert.length; i += batchSize) {
				...
				console.log(
					`  ✅ Inserted ${result.count} records in batch ${batchNumber}.`
				)
			}
